...
import { MyGeo, MyMaterial } from './d7/my'
...
export default (engine:any) => {
  // 灯光
  const directionalLight = new THREE.DirectionalLight(0xffff00, 50)
  ...
  loader.load(
    ...
    function (gltf) {
      ...
      targetMesh.scale.x = 2
      targetMesh.scale.y = 2
      targetMesh.scale.z = 2
      const params = {
        size: 1,
        height: 0,
      ...
      const geometry = new MyGeo(targetMesh.geometry)
      ...
      console.log('%c 🥛 size: ', 'font-size:20px;background-color: #93C0A4;color:#fff;', geometry.boundingBox)
